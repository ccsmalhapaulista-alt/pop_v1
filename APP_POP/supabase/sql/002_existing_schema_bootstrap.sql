...
drop trigger if exists trg_categorias_pop_updated_at on public.categorias_pop;
create trigger trg_categorias_pop_updated_at
before update on public.categorias_pop
for each row
execute function public.handle_updated_at();

drop trigger if exists trg_pop_itens_updated_at on public.pop_itens;
...
create index if not exists idx_pop_itens_tsv on public.pop_itens using gin (to_tsvector('portuguese', coalesce(titulo, '') || ' ' || coalesce(palavras_chave, '') || ' ' || coalesce(categoria, '')));
...
for select
to authenticated
using (public.is_admin_pop() or ((public.is_consulta_pop() or public.is_admin_pop()) and ativo = true));
...
for select
to authenticated
using (public.is_admin_pop() or ((public.is_consulta_pop() or public.is_admin_pop()) and status = 'PUBLICADO'));
